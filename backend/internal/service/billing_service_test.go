...
// 本地回退表不包含 glm/kimi 等国产模型族,改用两个不同的未知 Claude 子型号触发 fallback。
func TestGetModelPricing_FallbackWarnLoggedOncePerModel(t *testing.T) {
	svc := newTestBillingService()
	buf := captureStdLog(t)

	// claude-foo-bar 不在 LiteLLM,经 "claude" 关键字命中 sonnet-4 兜底价 → 触发 fallback warn。
	for i := 0; i < 5; i++ {
		pricing, err := svc.GetModelPricing("claude-foo-bar")
		require.NoError(t, err)
		require.NotNil(t, pricing)
	}

	got := strings.Count(buf.String(), "Using fallback pricing for model: claude-foo-bar")
...
func TestGetModelPricing_FallbackWarnPerModelNotGlobal(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		_, _ = svc.GetModelPricing("claude-foo-bar")
		_, _ = svc.GetModelPricing("CLAUDE-FOO-BAR") // 与上一行同模型(ToLower 后),去重后不再打
		_, _ = svc.GetModelPricing("claude-baz-qux")
	}

	out := buf.String()
	require.Equal(t, 1, strings.Count(out, "model: claude-foo-bar"), out)
	require.Equal(t, 1, strings.Count(out, "model: claude-baz-qux"), out)
	require.Equal(t, 0, strings.Count(out, "model: CLAUDE-FOO-BAR"), out) // 大写经 ToLower 归一,不应单独成行
}

// 回归:未知 Claude 子型号仍解析到 sonnet-4 兜底价(计费金额不变,防止日志改动掩盖未来计费回归)。
func TestGetModelPricing_UnknownClaudeFallsBackToSonnetPrice(t *testing.T) {
	svc := newTestBillingService()

	got, err := svc.GetModelPricing("claude-foo-bar")
	require.NoError(t, err)
	require.NotNil(t, got)

	// claude-sonnet-4:Input 3e-6 / Output 15e-6(见 TestGetFallbackPricing_FamilyMatching)。
	require.InDelta(t, 3e-6, got.InputPricePerToken, 1e-12)
	require.InDelta(t, 15e-6, got.OutputPricePerToken, 1e-12)
}
